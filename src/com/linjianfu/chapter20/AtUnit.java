...
public class AtUnit implements ProcessFiles.Strategy {
    ...
    static class TestMethods extends ArrayList<Method> {
        void addIfTestMethod(Method m) {
            ...
            if (!(m.getReturnType().equals(boolean.class) ||
                ...
                throw new RuntimeException("@Test method must " +
    ...
    private static Method checkForCreatorMethod(Method m) {
        ...
        if (!m.getReturnType().equals(testClass))
            throw new RuntimeException("@TestObjectCreate must " +
                    "return instance of Class to be tested");
    ...
    private static Method checkForCleanupMethod(Method m) {
        ...
        if (!m.getReturnType().equals(Void.class))
            throw new RuntimeException("@TestObjectCleanup must be void");
        if ((m.getModifiers() & Modifier.STATIC) < 1)
        ...
        if (m.getParameterTypes().length == 0 || m.getParameterTypes()[0] != testClass)
            throw new RuntimeException("@TestObjectCleanup must taken an argument " +
                    "of the tested type.");
        m.setAccessible(true);
        return m;
    }
